...
* Deve possuir a biblioteca `cryptography` instalada (ver passo abaixo).
...
A aplicação utiliza a biblioteca `cryptography` para a criptografia das mensagens. Instale-a com:
```
pip install cryptography
```
Caso esteja utilizando o gerenciador `uv`, use:
```
uv pip install cryptography
```
Ou, caso necessário, forçando no Python em uso:
```
